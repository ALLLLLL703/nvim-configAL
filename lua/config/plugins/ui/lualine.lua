require("lualine").setup({
	...
	sections = {
		lualine_a = { { "mode", icon_enabled = "mode", icon = { "" } } }, -- 模式名称（NORMAL, INSERT 等）
		...
		lualine_x = {
			...
			{ "encoding", icon_enabled = true, icon = "󰻐" },
			"filetype",
		},

		-- Section Y: 右侧第二个区域 (通常显示行和列号)
		lualine_y = { "progress", "datetime" }, -- 显示文件阅读进度百分比

		-- Section Z: 右侧第一个区域 (通常显示行和列号)
		lualine_z = { { "location", icon_enabled = true, icon = "" } }, -- 显示行号和列号 (例如 '10,20')
	},
